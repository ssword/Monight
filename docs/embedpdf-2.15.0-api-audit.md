...
- `AnnotationPluginConfig.autoOpenLinks` is disabled. In pinned release `2.15.0`, ready-made link rendering uses either a normal SVG hit area or a locked-mode HTML hit area. A capture-phase adapter matches those hit areas to the public `AnnotationScope.getAnnotations()` data, stops EmbedPDF selection/navigation, and forwards the untouched target to the originating Document's `activateDocumentTarget` Reader Action. XYZ destinations retain their vertical position as normalized Reading Position.
